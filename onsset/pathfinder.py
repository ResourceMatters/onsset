    ...
    * It assumes a gridded topology. In other words, it knows that
    ...
    if film:
        ...
        try:
            ...
            os.chdir(cwd)
    render_array = np.where(weights == 0, 0.001, weights)
    rendering = 1. / (2. * render_array)
    ...
    # around the set of origins that eventually envelops targets.
    ...
    test_time = 0
    # while len(halo) > 0:
    while (n_targets_remaining > 0) & (len(halo) > 0) & (max_connections > 0) & (max_capacity > 0):
        if (n_targets_remaining == 0) & (test_time == 0):
            print(time.ctime())
            test_time += 1
        iteration += 1
        if debug:
            if (n_targets_remaining > n_targets_remaining_update or
                ...
                print('\r {num} potential targets of {total} examined, {rem} remaining, {halo_len} to try '
                      .format(
    ...
    results = {'paths': paths, 'distance': distance, 'rendering': rendering, 'mv_dist': mv_distance, 'weights': weights,
               'new_connections_remaining': max_connections, 'new_capacity_remaining': max_capacity,
               'mv_distance_new': mv_distance_new}
    return results
...
def nb_trace_back(
    ...
    while distance_remaining > 0.:
        ...
        if distance_remaining > targets[target]:
            targets[target] = 0
            a = 1
            break
        ...
        #     distance_remaining == 0
    if len(path) == 1:
        total_ext_dist = (mv_distance[best_neighbor] + lowest_distance)
    else:
        total_ext_dist = mv_distance[best_neighbor] + distance[path[1]] + lowest_distance

    if (a == 1) or (total_ext_dist > 50):
        pass
    else:
        total_mv_line_dist = 0

        for i_loc, loc in enumerate(path):
            if i_loc == 0:
                total_mv_line_dist += mv_distance[loc]
                mv_distance[loc] = total_ext_dist
                if len(path) == 1:
                    mv_distance_new[loc] = lowest_distance
                else:
                    mv_distance_new[loc] = distance[path[1]] + lowest_distance

            else:
                total_mv_line_dist += distance[best_neighbor]
                mv_distance[loc] = mv_distance[best_neighbor] + distance[loc]
                #mv_distance_new[loc] = distance[loc]

        # Set new weights
        (row_here, col_here) = loc
        # Check each of the neighbors for the lowest distance to grid.
        neighbors = [
            ((row_here - 1, col_here), 1.),
            ((row_here + 1, col_here), 1.),
        ...
        for (neighbor, scale) in neighbors:
            if neighbor not in path:
                weights[neighbor] += 0.1 * mv_distance[loc] / 0.05

        ### End of additional weights

        # Add this new path.
        for i_loc, loc in enumerate(path):
            if distance[best_neighbor] > 0:
                pass
            paths[loc] = 1
...
def nb_loop(
    ...
    for (neighbor, scale) in neighbors:
        ...
        if distance[neighbor] == not_visited:
            ...
            max_connections -= new_connections[neighbor]
            max_capacity -= new_capacity[neighbor]
        ...
        if neighbor_distance < distance[neighbor]:
            ...
            if (neighbor[0] > 0 and
                ...
                n_new_locs += 1
    return n_new_locs, n_targets_remaining, max_connections, max_capacity
